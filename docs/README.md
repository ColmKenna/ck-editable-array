...
- Automatically mark the radio whose `value` matches the current field value as `checked` when entering edit mode.
- Listen to `change` events and update the underlying data only when a radio is selected (checked).
- Leave `value` attributes untouched (avoids clobbering option values) and sets `aria-checked` for accessibility.
