...
} from '../../src/components/ck-editable-array/ck-editable-array.styles';

describe('CkEditableArray Styles Module', () => {
  // Save original CSSStyleSheet for restoration
  const originalCSSStyleSheet: typeof CSSStyleSheet | undefined =
    globalThis.CSSStyleSheet;

  afterEach(() => {
    // Restore original CSSStyleSheet
    (
      globalThis as unknown as { CSSStyleSheet?: typeof CSSStyleSheet }
    ).CSSStyleSheet = originalCSSStyleSheet;
  });
  ...
  test('should return CSSStyleSheet or null depending on environment', () => {
    ...
    expect(
      ckEditableArraySheet === null ||
        ckEditableArraySheet instanceof CSSStyleSheet
    ).toBe(true);
  });
  ...
  test('should not throw when stylesheet module loads', async () => {
    // This ensures the module-level error handling works correctly
    await expect(
      import('../../src/components/ck-editable-array/ck-editable-array.styles')
  ...
  test('should return a working CSSStyleSheet when replaceSync succeeds', () => {
    ...
    expect(
      ckEditableArraySheet === null ||
        ckEditableArraySheet?.cssRules !== undefined
    ).toBe(true);
  });
